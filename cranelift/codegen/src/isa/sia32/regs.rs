...
impl Reg {
    ...
    pub const fn argument_index(self) -> Option<u8> {
        match self.0 { 1..=6 => Some(self.0 - 1), _ => None }
    }
}
...
pub const CALLER_SAVED: [Reg; 8] = [Reg(1), Reg(2), Reg(3), Reg(4), Reg(5), Reg(6), Reg(7), Reg(8)];
...
pub const fn mach_reg(index: u8) -> MachReg { MachReg::from_real_reg(preg(index)) }
...
pub const fn create_reg_environment() -> MachineEnv {
    let preferred_regs_by_class = [
        PRegSet::empty().with(preg(1)).with(preg(2)).with(preg(3)).with(preg(4))
