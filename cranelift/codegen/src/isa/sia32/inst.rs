...
use crate::ir::{self, ExternalName, TrapCode, Type};
...
impl MachInst for Inst {
    ...
    fn is_safepoint(&self) -> bool {
        self.is_trap() || matches!(self, Self::Call { .. } | Self::CallInd { .. })
    }
    ...
    fn emit(&self, code: &mut MachBuffer<Inst>, _info: &Self::Info, state: &mut EmitState) {
        ...
        match self {
            ...
            Self::Trap { code: trap_code } => {
                ...
                put_word(
                    code,
                    encode::trap(0).expect("SIA compiler trap opcode must be encodable"),
                );
            }
            Self::Mov { dst, src } => {
