...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum UnaryOp { Clz, Ctz, Cpop }

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum LoadOp { I8, U8, I16, U16, I32 }
...
pub(crate) fn rc_for_type(ty: &Type) -> CodegenResult<(&'static [RegClass], &'static [Type])> {
    match *ty {
        ...
        _ => Err(CodegenError::Unsupported(format!("SIA32 does not yet support SSA value type {ty}"))),
    }
}

fn arch_reg(reg: Reg) -> regs::Reg {
    let real = reg.to_real_reg().expect("SIA32 emission requires allocated real registers");
    assert_eq!(real.class(), RegClass::Int);
    regs::Reg::new(real.hw_enc()).expect("SIA32 real register encoding must be r0..r15")
}

fn put_word(code: &mut MachBuffer<Inst>, word: u16) { code.put2(word); }
...
fn emit_const32(code: &mut MachBuffer<Inst>, dst: regs::Reg, value: u32) {
    ...
    put_word(code, encode::li(dst, i32::from(digits[0])).expect("balanced top digit fits imm7"));
    for &digit in &digits[1..] {
        put_word(code, encode::shli(dst, 7).unwrap());
        if digit != 0 {
            put_word(code, encode::addi(dst, i32::from(digit)).expect("balanced digit fits imm7"));
        }
    }
}
...
fn emit_load_base_offset(code: &mut MachBuffer<Inst>, op: LoadOp, dst: regs::Reg, base: regs::Reg, offset: i32) {
    ...
    if dst != base {
        ...
        let scratch = regs::Reg::SCRATCH;
        assert!(base != scratch, "load r12,[r12+large] needs a second architectural scratch");
...
impl MachInst for Inst {
    ...
    fn get_operands(&mut self, collector: &mut impl OperandVisitor) {
        match self {
            ...
            Self::ReadFixedGpr { dst, .. } => collector.reg_def(dst),
            Self::WriteFixedGpr { src, .. } => collector.reg_use(src),
            ...
            | Self::Addi7 { dst, src, .. }
            | Self::Extend { dst, src, .. }
            | Self::AddImm { dst, src, .. } => { collector.reg_use(src); collector.reg_def(dst); }
            Self::Li7 { dst, .. } | Self::LoadConst32 { dst, .. } | Self::StackAddr { dst, .. } => collector.reg_def(dst),
            Self::Load { dst, base, .. } | Self::LoadBaseOffset { dst, base, .. } => { collector.reg_use(base); collector.reg_def(dst); }
            Self::Store { src, base, .. } | Self::StoreBaseOffset { src, base, .. } => { collector.reg_use(src); collector.reg_use(base); }
            Self::IndexedLoad { dst, base, index } => { collector.reg_use(base); collector.reg_use(index); collector.reg_def(dst); }
            Self::IndexedStore { src, base, index } => { collector.reg_use(src); collector.reg_use(base); collector.reg_use(index); }
    ...
    fn gen_dummy_use(reg: Reg) -> Self { Self::DummyUse { reg } }
    fn rc_for_type(ty: &Type) -> CodegenResult<(&[RegClass], &[Type])> { rc_for_type(ty) }
    fn canonical_type_for_rc(rc: RegClass) -> Type { match rc { RegClass::Int => I32, RegClass::Float | RegClass::Vector => unreachable!("SIA32 has only integer registers") } }
    fn gen_jump(target: MachLabel) -> Self { Self::Jump { target } }
    fn gen_imm_u64(value: u64, dst: Writable<Reg>) -> Option<Self> { u32::try_from(value).ok().map(|value| Self::LoadConst32 { dst, value }) }
...
impl MachInstEmitState<Inst> for EmitState {
    fn new(abi: &Callee<Sia32MachineDeps>, ctrl_plane: ControlPlane) -> Self {
        Self { ctrl_plane, frame_layout: abi.frame_layout().clone(), user_stack_map: None }
    }
    fn pre_safepoint(&mut self, user_stack_map: Option<ir::UserStackMap>) { self.user_stack_map = user_stack_map; }
    fn ctrl_plane_mut(&mut self) -> &mut ControlPlane { &mut self.ctrl_plane }
    fn take_ctrl_plane(self) -> ControlPlane { self.ctrl_plane }
    fn frame_layout(&self) -> &FrameLayout { &self.frame_layout }
}
...
impl MachInstEmit for Inst {
    ...
    fn emit(&self, code: &mut MachBuffer<Inst>, _info: &Self::Info, state: &mut EmitState) {
        ...
        match self {
            ...
            Self::Unary { op, dst, src } => {
                let word = match op { UnaryOp::Clz => encode::clz(arch_reg(dst.to_reg()), arch_reg(*src)), UnaryOp::Ctz => encode::ctz(arch_reg(dst.to_reg()), arch_reg(*src)), UnaryOp::Cpop => encode::cpop(arch_reg(dst.to_reg()), arch_reg(*src)) };
                put_word(code, word);
            }
            Self::TwoOp { op, dst, lhs, rhs } => {
                let d = arch_reg(dst.to_reg()); let l = arch_reg(*lhs); let r = arch_reg(*rhs);
                if d != l { put_word(code, encode::mov(d, l)); }
                let word = match op {
                    TwoOp::Sub => encode::sub(d,r), TwoOp::Addo => encode::addo(d,r), TwoOp::Subo => encode::subo(d,r),
                    TwoOp::CmpEq => encode::cmpeq(d,r), TwoOp::CmpLt => encode::cmplt(d,r), TwoOp::CmpLtu => encode::cmpltu(d,r),
            ...
            Self::ShiftImm { op, dst, src, amount } => {
                ...
                let word = match op { TwoOp::Shl => encode::shli(d,*amount), TwoOp::Shr => encode::shri(d,*amount), TwoOp::Sar => encode::sari(d,*amount), _ => panic!("invalid SIA32 immediate shift") }.expect("shift amount validated");
                put_word(code, word);
            }
            ...
            Self::LoadStack { dst, mem, ty } => {
                ...
                emit_load_base_offset(code,load_op(*ty),arch_reg(dst.to_reg()),regs::Reg::SP,off);
            }
            ...
            Self::LoadBaseOffset { dst, base, offset, ty } => emit_load_base_offset(code,load_op(*ty),arch_reg(dst.to_reg()),arch_reg(*base),*offset),
            Self::StoreBaseOffset { src, base, offset, ty } => emit_store_base_offset(code,store_op(*ty),arch_reg(*src),arch_reg(*base),*offset),
            Self::Extend { dst, src, signed, from_bits, to_bits } => {
                assert!(*from_bits <= *to_bits && *to_bits <= 32 && *from_bits > 0);
                let d=arch_reg(dst.to_reg()); let s=arch_reg(*src);
                if d != s { put_word(code,encode::mov(d,s)); }
                if from_bits < to_bits {
                    let shift=32-*from_bits;
                    if shift != 0 {
                        put_word(code,encode::shli(d,shift).unwrap());
                        put_word(code,if *signed { encode::sari(d,shift).unwrap() } else { encode::shri(d,shift).unwrap() });
                    }
                }
            }
            Self::AddImm { dst, src, imm } => emit_add_imm32(code,arch_reg(dst.to_reg()),arch_reg(*src),*imm),
            Self::SpAdjust { amount } => emit_add_imm32(code,regs::Reg::SP,regs::Reg::SP,*amount),
            Self::StackLowerBoundTrap { .. } => panic!("SIA32 stack probing is not implemented by the frozen ABI"),
    ...
    fn pretty_print_inst(&self, _state: &mut Self::State) -> String { format!("{self:?}") }
}
...
mod tests {
    ...
    fn scalar_and_i64_register_representation_is_32_bit_native(){
        for ty in [I8,I16,I32]{ let (rcs,tys)=rc_for_type(&ty).unwrap(); assert_eq!(rcs,&[RegClass::Int]); assert_eq!(tys,&[I32]); }
        let (rcs,tys)=rc_for_type(&I64).unwrap(); assert_eq!(rcs,&[RegClass::Int,RegClass::Int]); assert_eq!(tys,&[I32,I32]);
    }
    #[test]
    fn unsupported_value_classes_fail_explicitly(){ assert!(rc_for_type(&F32).is_err()); assert!(rc_for_type(&I128).is_err()); }
    #[test]
    fn move_and_memory_classification_is_exact(){
        let mov=Inst::Mov{dst:w(1),src:mach_reg(2)}; assert_eq!(MachInst::is_move(&mov),Some((w(1),mach_reg(2)))); assert!(!mov.is_mem_access());
        let load=Inst::Load{op:LoadOp::I32,dst:w(1),base:mach_reg(2)}; assert!(load.is_mem_access());
    }
    #[test]
    fn arbitrary_i32_constants_have_bounded_exact_reconstruction(){
        for value in [0u32,1,63,64,127,128,0x7fff_ffff,0x8000_0000,0xffff_ffff,0xdead_beef] {
            let digits=const_digits(value); assert!(!digits.is_empty()); assert!(digits.len()<=5); assert!(digits.iter().all(|d|(-64..=63).contains(d)));
    ...
    fn pseudos_report_conservative_sizes(){
        let op=Inst::TwoOp{op:TwoOp::Sub,dst:w(1),lhs:mach_reg(2),rhs:mach_reg(3)}; assert_eq!(op.encoded_worst_case_size(),4);
        let c=Inst::LoadConst32{dst:w(1),value:0xdead_beef}; assert_eq!(c.encoded_worst_case_size(),18);
    }
    #[test]
    fn trap_and_nop_bytes_are_exact(){ assert_eq!(Inst::TRAP_OPCODE,&[0x0f,0xc0]); assert_eq!(Inst::gen_nop_units(),vec![vec![0xff,0xcf]]); }
}
